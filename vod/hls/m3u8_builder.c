...
#define M3U8_EXT_MEDIA_BASE                                                    \
  "#EXT-X-MEDIA:TYPE=%s,GROUP-ID=\"%s%uD\",NAME=\"%V\","
...
#define M3U8_STREAM_TAG_SUBTITLES                                              \
  ",SUBTITLES=\"" M3U8_EXT_MEDIA_GROUP_ID_SUBTITLES "%uD\""
#define M3U8_STREAM_TAG_CLOSED_CAPTIONS                                        \
  ",CLOSED-CAPTIONS=\"" M3U8_EXT_MEDIA_GROUP_ID_CLOSED_CAPTIONS "%uD\""
...
static const char m3u8_stream_inf_video[] =
    "#EXT-X-STREAM-INF:PROGRAM-ID=1,BANDWIDTH=%uD,RESOLUTION=%uDx%uD,FRAME-"
    "RATE=%uD.%03uD,CODECS=\"%V";
static const char m3u8_stream_inf_audio[] =
    "#EXT-X-STREAM-INF:PROGRAM-ID=1,BANDWIDTH=%uD,CODECS=\"%V";
static const char m3u8_average_bandwidth[] = ",AVERAGE-BANDWIDTH=%uD";
static const char m3u8_iframe_stream_inf[] =
    "#EXT-X-I-FRAME-STREAM-INF:BANDWIDTH=%uD,RESOLUTION=%uDx%uD,CODECS=\"%V\","
    "URI=\"";
...
static const char encryption_key_tag_key_format_versions[] =
    ",KEYFORMATVERSIONS=\"";
...
static u_char *m3u8_builder_format_double(u_char *p, uint32_t n,
                                          uint32_t scale) {
  int cur_digit;
  int int_n = n / scale;
...
static u_char *
m3u8_builder_append_segment_name(u_char *p, vod_str_t *base_url,
                                 vod_str_t *segment_file_name_prefix,
                                 uint32_t segment_index, vod_str_t *suffix) {
  p = vod_copy(p, base_url->data, base_url->len);
  p = vod_copy(p, segment_file_name_prefix->data,
               segment_file_name_prefix->len);
  *p++ = '-';
...
static u_char *m3u8_builder_append_extinf_tag(u_char *p, uint32_t duration,
                                              uint32_t scale) {
...
static void m3u8_builder_append_iframe_string(void *context,
  ...
  ctx->p = m3u8_builder_append_segment_name(ctx->p, ctx->base_url,
                                            ctx->segment_file_name_prefix,
                                            segment_index, &ctx->name_suffix);
}

static vod_status_t
m3u8_builder_build_tracks_spec(request_context_t *request_context,
                               media_set_t *media_set, vod_str_t *suffix,
                               vod_str_t *result) {
  ...
  // get the result size
  result_size = suffix->len + (sizeof("-v") - 1 + VOD_INT32_LEN) *
                                  media_set->total_track_count;
  if (media_set->has_multi_sequences) {
    result_size +=
        (sizeof("-f") - 1 + VOD_INT32_LEN) * media_set->total_track_count;
  }

  // allocate the result buffer and the track ptrs array
  tracks =
      vod_alloc(request_context->pool,
                sizeof(tracks[0]) * media_set->total_track_count + result_size);
  if (tracks == NULL) {
  ...
  for (cur_track_ptr = tracks, cur_track = media_set->filtered_tracks;
       cur_track_ptr < tracks_end; cur_track_ptr++, cur_track++) {
  ...
  p = manifest_utils_append_tracks_spec(p, tracks, media_set->total_track_count,
  ...
  if (result->len > result_size) {
    vod_log_error(VOD_LOG_ERR, request_context->log, 0,
                  "m3u8_builder_build_tracks_spec: result length %uz exceeded "
                  "allocated length %uz",
...
vod_status_t m3u8_builder_build_iframe_playlist(
    request_context_t *request_context, m3u8_config_t *conf,
    hls_mpegts_muxer_conf_t *muxer_conf, vod_str_t *base_url,
    media_set_t *media_set, vod_str_t *result) {
  ...
  // 2. SAMPLE-AES - the layout of the TS files is not known in advance due to
  ...
  rc = m3u8_builder_build_tracks_spec(request_context, media_set,
                                      &m3u8_ts_suffix, &ctx.name_suffix);
  if (rc != VOD_OK) {
    return rc;
  }
  ...
  if (segmenter_conf->align_to_key_frames) {
    rc = segmenter_get_segment_durations_accurate(
        request_context, segmenter_conf, media_set, NULL, MEDIA_TYPE_NONE,
        &segment_durations);
  } else {
  ...
  iframe_length = sizeof("#EXTINF:.000,\n") - 1 +
                  vod_get_int_print_len(vod_div_ceil(duration_millis, 1000)) +
                  sizeof(byte_range_tag_format) + VOD_INT32_LEN +
                  vod_get_int_print_len(MAX_FRAME_SIZE) -
                  (sizeof("%uD%uD") - 1) + base_url->len +
                  conf->segment_file_name_prefix.len + 1 +
  ...
  // fill out the buffer
  ctx.p = vod_copy(result->data, conf->iframes_m3u8_header,
  ...
  if (media_set->sequences[0].video_key_frame_count > 0) {
    ...
    rc = hls_muxer_simulate_get_iframes(
        request_context, &segment_durations, muxer_conf, &encryption_params,
        media_set, m3u8_builder_append_iframe_string, &ctx);
    if (rc != VOD_OK) {
      return rc;
    }
  }
  ...
  if (result->len > result_size) {
    vod_log_error(VOD_LOG_ERR, request_context->log, 0,
                  "m3u8_builder_build_iframe_playlist: result length %uz "
                  "exceeded allocated length %uz",
                  result->len, result_size);
    return VOD_UNEXPECTED;
  }
...
#if (NGX_HAVE_OPENSSL_EVP)
static vod_status_t m3u8_builder_write_psshs(request_context_t *request_context,
                                             drm_info_t *drm_info,
                                             vod_str_t *result) {
  ...
  for (cur_info = drm_info->pssh_array.first;
       cur_info < drm_info->pssh_array.last; cur_info++) {
  ...
  if (result->len != result_size) {
    vod_log_error(VOD_LOG_ERR, request_context->log, 0,
                  "m3u8_builder_write_psshs: result length %uz exceeded "
                  "allocated length %uz",
                  result->len, result_size);
    return VOD_UNEXPECTED;
  }
...
vod_status_t m3u8_builder_build_index_playlist(
    request_context_t *request_context, m3u8_config_t *conf,
    vod_str_t *base_url, vod_str_t *segments_base_url,
    hls_encryption_params_t *encryption_params, vod_uint_t container_format,
    media_set_t *media_set, vod_str_t *result) {
  segment_durations_t segment_durations;
  ...
  if (media_set->track_count[MEDIA_TYPE_VIDEO] != 0 ||
    ...
    container_format =
        HLS_CONTAINER_MPEGTS; // do not output any fmp4-specific tags
    suffix = &m3u8_vtt_suffix;
  }

  rc = m3u8_builder_build_tracks_spec(request_context, media_set, suffix,
                                      &name_suffix);
  if (rc != VOD_OK) {
    return rc;
  }
  ...
  rc = segmenter_conf->get_segment_durations(request_context, segmenter_conf,
                                             media_set, NULL, MEDIA_TYPE_NONE,
                                             &segment_durations);
  if (rc != VOD_OK) {
    return rc;
  }
  ...
  segment_length = sizeof("#EXTINF:.000,\n") - 1 +
                   vod_get_int_print_len(vod_div_ceil(duration_millis, 1000)) +
                   segments_base_url->len + conf->segment_file_name_prefix.len +
                   1 + vod_get_int_print_len(last_segment_index) +
                   name_suffix.len;

  result_size =
      sizeof(M3U8_HEADER_PART1) + VOD_INT64_LEN + sizeof(M3U8_HEADER_EVENT) +
      ...
      (sizeof(m3u8_map_prefix) - 1 + base_url->len +
       conf->init_file_name_prefix.len + sizeof(m3u8_clip_index) - 1 +
       VOD_INT32_LEN + name_suffix.len + sizeof(m3u8_map_suffix) - 1) *
          (segment_durations.discontinuities + 1) +
      sizeof(m3u8_footer);

  if (encryption_type != HLS_ENC_NONE) {
    result_size += sizeof(encryption_key_tag_method) - 1 +
                   sizeof(encryption_type_sample_aes_cenc) - 1 +
                   sizeof(encryption_key_tag_uri) - 1 + 2; // '"', '\n'
    ...
    if (encryption_params->key_uri.len != 0) {
      ...
      rc = m3u8_builder_write_psshs(request_context,
                                    media_set->sequences[0].drm_info, &psshs);
      if (rc != VOD_OK) {
        return rc;
      }

      result_size += sizeof(sample_aes_cenc_uri_prefix) +
    ...
    if (encryption_params->return_iv) {
      result_size += sizeof(encryption_key_tag_iv) - 1 +
                     sizeof(encryption_params->iv_buf) * 2;
    }

    if (conf->encryption_key_format.len != 0) {
      result_size += sizeof(encryption_key_tag_key_format) + // '"'
                     conf->encryption_key_format.len;
    }

    if (conf->encryption_key_format_versions.len != 0) {
      result_size += sizeof(encryption_key_tag_key_format_versions) + // '"'
  ...
  // Note: scaling first to 'scale' so that target duration will always be
  // round(max(manifest durations))
  scale = conf->m3u8_version >= 3 ? 1000 : 1;
  max_segment_duration =
      rescale_time(max_segment_duration, segment_durations.timescale, scale);
  ...
  // write the header
  p = vod_sprintf(result->data, M3U8_HEADER_PART1, max_segment_duration);
  ...
  if (encryption_type != HLS_ENC_NONE) {
    p = vod_copy(p, encryption_key_tag_method,
                 sizeof(encryption_key_tag_method) - 1);
    switch (encryption_type) {
    case HLS_ENC_SAMPLE_AES:
      p = vod_copy(p, encryption_type_sample_aes,
                   sizeof(encryption_type_sample_aes) - 1);
      break;

    case HLS_ENC_SAMPLE_AES_CENC:
      p = vod_copy(p, encryption_type_sample_aes_cenc,
                   sizeof(encryption_type_sample_aes_cenc) - 1);
      break;

    default: // HLS_ENC_AES_128
      p = vod_copy(p, encryption_type_aes_128,
                   sizeof(encryption_type_aes_128) - 1);
      break;
    }
    ...
    *p++ = '"';
    ...
    // keyformat
    if (conf->encryption_key_format.len != 0) {
      p = vod_copy(p, encryption_key_tag_key_format,
                   sizeof(encryption_key_tag_key_format) - 1);
      p = vod_copy(p, conf->encryption_key_format.data,
                   conf->encryption_key_format.len);
      *p++ = '"';
    }
    ...
    // keyformatversions
    if (conf->encryption_key_format_versions.len != 0) {
      p = vod_copy(p, encryption_key_tag_key_format_versions,
                   sizeof(encryption_key_tag_key_format_versions) - 1);
      p = vod_copy(p, conf->encryption_key_format_versions.data,
                   conf->encryption_key_format_versions.len);
      *p++ = '"';
    }
  ...
  p = vod_sprintf(p, M3U8_HEADER_PART2,
                  container_format == HLS_CONTAINER_FMP4 ? 6
                                                         : conf->m3u8_version,
  ...
  if (container_format == HLS_CONTAINER_FMP4) {
    ...
    p = vod_copy(p, base_url->data, base_url->len);
    p = vod_copy(p, conf->init_file_name_prefix.data,
  ...
  for (cur_item = segment_durations.items; cur_item < last_item; cur_item++) {
    ...
    p = m3u8_builder_append_extinf_tag(
        p, rescale_time(cur_item->duration, segment_durations.timescale, scale),
        scale);
    extinf.len = p - extinf.data;
    p = m3u8_builder_append_segment_name(p, segments_base_url,
                                         &conf->segment_file_name_prefix,
                                         segment_index, &name_suffix);
    segment_index++;
  ...
  if (result->len > result_size) {
    vod_log_error(VOD_LOG_ERR, request_context->log, 0,
                  "m3u8_builder_build_index_playlist: result length %uz "
                  "exceeded allocated length %uz",
                  result->len, result_size);
    return VOD_UNEXPECTED;
  }
...
static uint32_t
m3u8_builder_get_audio_codec_count(adaptation_sets_t *adaptation_sets,
  ...
  last_adaptation_set =
      cur_adaptation_set + adaptation_sets->count[MEDIA_TYPE_AUDIO];
...
static u_char *m3u8_builder_append_index_url(u_char *p, vod_str_t *prefix,
  ...
  if (base_url->len != 0) {
    ...
    if (p[-1] != '/') {
      if (main_track->file_info.uri.len != 0 &&
          (sub_track == NULL || vod_str_equals(main_track->file_info.uri,
                                               sub_track->file_info.uri))) {
        p = vod_copy(p, main_track->file_info.uri.data,
                     main_track->file_info.uri.len);
        write_sequence_index = FALSE; // no need to pass the sequence index
                                      // since we have a direct uri
      } else {
  ...
  p = manifest_utils_append_tracks_spec(p, tracks, MEDIA_TYPE_COUNT,
...
static size_t
m3u8_builder_closed_captions_get_size(media_set_t *media_set,
                                      request_context_t *request_context) {
  media_closed_captions_t *closed_captions;
  size_t result = 0;
  size_t base;

  base = sizeof(M3U8_EXT_MEDIA_BASE) - 1 +
         sizeof(M3U8_EXT_MEDIA_TYPE_CLOSED_CAPTIONS) - 1 +
         sizeof(M3U8_EXT_MEDIA_GROUP_ID_CLOSED_CAPTIONS) - 1 + VOD_INT32_LEN +
         sizeof(M3U8_EXT_MEDIA_LANG) - 1 + sizeof(M3U8_EXT_MEDIA_INSTREAM_ID) -
         1 + sizeof(M3U8_EXT_MEDIA_DEFAULT) - 1;

  for (closed_captions = media_set->closed_captions;
       closed_captions < media_set->closed_captions_end; closed_captions++) {
    result += base + closed_captions->id.len + closed_captions->label.len +
              closed_captions->language.len + sizeof("\n") - 1;
  }

  return result + sizeof("\n") - 1;
}

static u_char *m3u8_builder_closed_captions_write(u_char *p,
                                                  media_set_t *media_set) {
  media_closed_captions_t *closed_captions;
  uint32_t index = 0;
  bool_t is_default;
  ...
  for (closed_captions = media_set->closed_captions;
       closed_captions < media_set->closed_captions_end; closed_captions++) {
    p = vod_sprintf(p, M3U8_EXT_MEDIA_BASE, M3U8_EXT_MEDIA_TYPE_CLOSED_CAPTIONS,
                    M3U8_EXT_MEDIA_GROUP_ID_CLOSED_CAPTIONS, index,
    ...
    if (is_default) {
      p = vod_copy(p, M3U8_EXT_MEDIA_DEFAULT,
                   sizeof(M3U8_EXT_MEDIA_DEFAULT) - 1);
    } else {
      p = vod_copy(p, M3U8_EXT_MEDIA_NON_DEFAULT,
                   sizeof(M3U8_EXT_MEDIA_NON_DEFAULT) - 1);
    }

    p = vod_sprintf(p, M3U8_EXT_MEDIA_INSTREAM_ID,
                    (vod_str_t *)&closed_captions->id);
...
static size_t m3u8_builder_ext_x_media_tags_get_size(
    adaptation_sets_t *adaptation_sets, vod_str_t *base_url,
    size_t base_url_len, media_set_t *media_set, uint32_t media_type) {
  ...
  result =
      ...
      (sizeof(M3U8_EXT_MEDIA_BASE) - 1 + VOD_INT32_LEN +
       ...
       sizeof(M3U8_EXT_MEDIA_GROUP_ID_AUDIO) - 1 + sizeof(M3U8_EXT_MEDIA_LANG) -
       1 + sizeof(M3U8_EXT_MEDIA_DEFAULT) - 1 +
       sizeof(M3U8_EXT_MEDIA_CHANNELS) - 1 + VOD_INT32_LEN +
       sizeof(M3U8_EXT_MEDIA_URI) - 1 + base_url_len + sizeof("\"\n") - 1) *
          (adaptation_sets->count[media_type]);

  first_adaptation_set = adaptation_sets->first_by_type[media_type];
  last_adaptation_set =
      first_adaptation_set + adaptation_sets->count[media_type];
  for (adaptation_set = first_adaptation_set;
       adaptation_set < last_adaptation_set; adaptation_set++) {
...
static u_char *m3u8_builder_ext_x_media_tags_write(
    u_char *p, adaptation_sets_t *adaptation_sets, m3u8_config_t *conf,
    vod_str_t *base_url, media_set_t *media_set, uint32_t media_type) {
  adaptation_set_t *first_adaptation_set;
  adaptation_set_t *last_adaptation_set;
  adaptation_set_t *adaptation_set;
  ...
  *p++ = '\n';
  ...
  for (adaptation_set = first_adaptation_set;
    ...
    if (media_type == MEDIA_TYPE_AUDIO) {
      group_index =
          tracks[media_type]->media_info.codec_id - VOD_CODEC_ID_AUDIO;
    } else {
      group_index = 0;
    }
    ...
    p = vod_sprintf(p, M3U8_EXT_MEDIA_BASE, type, group_id, group_index, label);
...
static u_char *m3u8_builder_write_video_range(u_char *p,
...
static u_char *m3u8_builder_write_variants(u_char *p,
  ...
  uint32_t muxed_tracks =
      adaptation_set->type == ADAPTATION_TYPE_MUXED ? MEDIA_TYPE_COUNT : 1;

  vod_memzero(tracks, sizeof(tracks));

  for (cur_track_ptr = adaptation_set->first;
       cur_track_ptr < adaptation_set->last; cur_track_ptr += muxed_tracks) {
    ...
    if (muxed_tracks == MEDIA_TYPE_COUNT) {
      ...
      // Note: this is ok because the adaptation types enum is aligned with
      // media types
    ...
    if (tracks[MEDIA_TYPE_VIDEO] != NULL) {
      ...
      p = vod_sprintf(p, m3u8_stream_inf_video, bitrate,
                      (uint32_t)video->u.video.width,
                      (uint32_t)video->u.video.height,
                      (uint32_t)(video->timescale / video->min_frame_duration),
                      (uint32_t)((((uint64_t)video->timescale * 1000) /
                                  video->min_frame_duration) %
                                 1000),
                      &video->codec_name);
      if (audio != NULL) {
        *p++ = ',';
      ...
      avg_bitrate = audio->avg_bitrate;
      p = vod_sprintf(p, m3u8_stream_inf_audio, audio->bitrate,
                      &audio->codec_name);
    }
    ...
    *p++ = '\"';
    ...
    if (adaptation_sets->count[ADAPTATION_TYPE_AUDIO] > 0 &&
        adaptation_sets->total_count > 1) {
      p = vod_sprintf(p, M3U8_STREAM_TAG_AUDIO,
                      group_audio_track->media_info.codec_id -
                          VOD_CODEC_ID_AUDIO);
    }
    ...
    if (media_set->closed_captions < media_set->closed_captions_end) {
      ...
      p = vod_copy(p, M3U8_STREAM_TAG_NO_CLOSED_CAPTIONS,
                   sizeof(M3U8_STREAM_TAG_NO_CLOSED_CAPTIONS) - 1);
    }
    *p++ = '\n';
...
static u_char *
m3u8_builder_write_iframe_variants(u_char *p, adaptation_set_t *adaptation_set,
                                   m3u8_config_t *conf, vod_str_t *base_url,
                                   media_set_t *media_set) {
  media_track_t **cur_track_ptr;
  media_track_t *tracks[MEDIA_TYPE_COUNT];
  media_info_t *video;
  ...
  for (cur_track_ptr = adaptation_set->first;
    ...
    p = vod_sprintf(p, m3u8_iframe_stream_inf, video->u.video.key_frame_bitrate,
                    (uint32_t)video->u.video.width,
                    (uint32_t)video->u.video.height, &video->codec_name);

    // Note: while it is possible to use only the video track here, sending the
    // audio
    //		makes the iframe list reference the same segments as the media
    // playlist
    p = m3u8_builder_append_index_url(p, &conf->iframes_file_name_prefix,
                                      media_set, tracks, base_url);
    *p++ = '\"';
...
vod_status_t m3u8_builder_build_master_playlist(
    request_context_t *request_context, m3u8_config_t *conf,
    vod_uint_t encryption_method, vod_str_t *base_url, media_set_t *media_set,
    vod_str_t *result) {
  adaptation_sets_t adaptation_sets;
  ...
  flags = ADAPTATION_SETS_FLAG_SINGLE_LANG_TRACK |
          ADAPTATION_SETS_FLAG_MULTI_AUDIO_CODEC;
  if (!conf->force_unmuxed_segments &&
      encryption_method != HLS_ENC_SAMPLE_AES_CENC) {
    flags |= ADAPTATION_SETS_FLAG_MUXED;
  }

  rc = manifest_utils_get_adaptation_sets(request_context, media_set, flags,
                                          &adaptation_sets);
  if (rc != VOD_OK) {
    return rc;
  }
  ...
  iframe_playlist =
      conf->output_iframes_playlist &&
      (media_set->type == MEDIA_SET_VOD || media_set->is_live_event) &&
      media_set->timing.total_count <= 1 && encryption_method == HLS_ENC_NONE &&
  ...
  base_url_len = base_url->len + 1 + conf->index_file_name_prefix.len + // 1 = /
                 MANIFEST_UTILS_TRACKS_SPEC_MAX_SIZE + sizeof(m3u8_url_suffix) -
                 1;

  result_size = sizeof(m3u8_header);

  max_video_stream_inf =
      sizeof(m3u8_stream_inf_video) - 1 + 5 * VOD_INT32_LEN +
      MAX_CODEC_NAME_SIZE + MAX_CODEC_NAME_SIZE + 1 + // 1 = ,
      sizeof(m3u8_average_bandwidth) - 1 + VOD_INT32_LEN +
      sizeof(M3U8_VIDEO_RANGE_SDR) - 1 + sizeof("\"\n\n") - 1;

  alternative_audio = adaptation_sets.count[ADAPTATION_TYPE_AUDIO] > 0 &&
                      adaptation_sets.total_count > 1;

  if (alternative_audio) {
    // alternative audio
    // Note: in case of audio only, the first track is printed twice - once as
    // #EXT-X-STREAM-INF
    ...
    variant_set_count = m3u8_builder_get_audio_codec_count(&adaptation_sets,
                                                           audio_codec_tracks);
  } else {
    variant_set_count = 1;
  }
  ...
  if (adaptation_sets.count[ADAPTATION_TYPE_SUBTITLE] > 0) {
    // subtitles
    result_size += m3u8_builder_ext_x_media_tags_get_size(
        &adaptation_sets, base_url, base_url_len, media_set,
        MEDIA_TYPE_SUBTITLE);

    max_video_stream_inf +=
        sizeof(M3U8_STREAM_TAG_SUBTITLES) - 1 + VOD_INT32_LEN;
  }

  if (media_set->closed_captions < media_set->closed_captions_end) {
    result_size +=
        m3u8_builder_closed_captions_get_size(media_set, request_context);
  ...
  // variants
  muxed_tracks = adaptation_sets.first->type == ADAPTATION_TYPE_MUXED
                     ? MEDIA_TYPE_COUNT
                     : 1;
  ...
  if (base_url->len != 0) {
    for (cur_track_ptr = adaptation_sets.first->first;
      ...
      variant_set_size +=
          vod_max(cur_track->file_info.uri.len, media_set->uri.len);
    }
  }
  ...
  if (iframe_playlist) {
    result_size +=
        (sizeof(m3u8_iframe_stream_inf) - 1 + 3 * VOD_INT32_LEN +
         MAX_CODEC_NAME_SIZE + sizeof("\"\n\n") - 1 + base_url_len -
         conf->index_file_name_prefix.len + conf->iframes_file_name_prefix.len +
  ...
  if (alternative_audio) {
    ...
    p = m3u8_builder_ext_x_media_tags_write(p, &adaptation_sets, conf, base_url,
                                            media_set, MEDIA_TYPE_AUDIO);
  }

  if (adaptation_sets.count[ADAPTATION_TYPE_SUBTITLE] > 0) {
    // output subtitles
    p = m3u8_builder_ext_x_media_tags_write(p, &adaptation_sets, conf, base_url,
                                            media_set, MEDIA_TYPE_SUBTITLE);
  }
  ...
  if (variant_set_count > 1) {
    ...
    for (cur_track_ptr = audio_codec_tracks;
         cur_track_ptr < last_audio_codec_track; cur_track_ptr++) {
      p = m3u8_builder_write_variants(p, &adaptation_sets, conf, base_url,
                                      media_set, *cur_track_ptr);
    }
  } else {
    p = m3u8_builder_write_variants(
        p, &adaptation_sets, conf, base_url, media_set,
        alternative_audio
            ? adaptation_sets.first_by_type[ADAPTATION_TYPE_AUDIO]->first[0]
            : NULL);
  }

  // iframes
  if (iframe_playlist) {
    p = m3u8_builder_write_iframe_variants(p, adaptation_sets.first, conf,
                                           base_url, media_set);
  }
  ...
  if (result->len > result_size) {
    vod_log_error(VOD_LOG_ERR, request_context->log, 0,
                  "m3u8_builder_build_master_playlist: result length %uz "
                  "exceeded allocated length %uz",
                  result->len, result_size);
    return VOD_UNEXPECTED;
  }
...
void m3u8_builder_init_config(m3u8_config_t *conf,
  ...
  conf->iframes_m3u8_header_len =
      vod_snprintf(conf->iframes_m3u8_header,
